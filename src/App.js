...
import { filterOnlyFavorite } from './services/favorite.service';
import './App.scss';
...
class App extends Component {
  ...
  search = async(options = {}) => {
    ...
    const charactersAsync = callAPI();
    charactersAsync.done(() => this.setState({ loading: false }));
    return charactersAsync;
  }

  favoriteFilter = (isFavoriteEnable) => {
    if (isFavoriteEnable) {
      const filtered = filterOnlyFavorite(this.state.characters);
      ...
      <div className="App">
        ...
        <nav className="navbar App-navbar">
          <ul className="nav navbar-nav">
            <li className="active"><a href="/"><span className="h4">Personagens</span></a></li>
            {/*<li><a href="#"><span className="h4">Comics</span></a></li>*/}
          </ul>
        </nav>
        <Filters ref={filters => this.filters = filters} favoriteFilter={this.favoriteFilter} onApply={this.applyFilters} onReset={this.resetFilters} />
